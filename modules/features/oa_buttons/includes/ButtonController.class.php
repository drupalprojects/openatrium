...
class ButtonController extends EntityAPIControllerExportable {
  ...
  public function save($entity) {
    ...
    field_attach_presave('oa_button', $entity);
    module_invoke_all('entity_presave', $entity, 'oa_button');

    try {
      if (!$entity->is_new) {
        drupal_write_record('oa_buttons', $entity, 'bid');
        field_attach_update('oa_button', $entity);
        module_invoke_all('entity_update', $entity, 'oa_button');
      }
      else {
        drupal_write_record('oa_buttons', $entity);
        field_attach_insert('oa_button', $entity);
        module_invoke_all('entity_insert', $entity, 'oa_button');
      }
      ...
      watchdog_exception('oa_button', $e);
    }
  ...
  public function view($entities, $view_mode = 'full', $langcode = NULL, $page = NULL) {
    ...
    foreach ($entities as $entity) {
      ...
      $type = 'oa_button';
      drupal_alter(array('oa_buttons_view', 'entity_view'), $entity->content, $type);
  ...
  public function buildContent($entity, $view_mode = 'full', $langcode = NULL) {
    ...
    field_attach_prepare_view('oa_button', array($entity->bid => $entity), $view_mode, $langcode);
    entity_prepare_view('oa_button', array($entity->bid => $entity), $langcode);
    $entity->content += field_attach_view('oa_button', $entity, $view_mode, $langcode);

    // Allow modules to make their own additions to the entity.
    module_invoke_all('oa_button_view', $entity, $view_mode, $langcode);
    module_invoke_all('entity_view', $entity, 'oa_button', $view_mode, $langcode);
  ...
  public function delete($bids) {
    $transaction = db_transaction();
    if (!empty($bids)) {
      $entities = oa_buttons_load_multiple($bids, array());

      try {
        foreach ($entities as $bid => $entity) {
          // Call the entity-specific callback (if any):
          module_invoke_all('entity_delete', $entity, 'oa_button');
          field_attach_delete('oa_button', $entity);
        }

        // Delete after calling hooks so that they can query entity tables as needed.
        db_delete('oa_buttons')
      ...
      entity_get_controller('oa_button')->resetCache();
    }
  }
